...
- The docs describe the fuller contract needed by the frontend:
  ...
  - server: `roomCreated`, `roomJoined`, `roomState`, `gameStarting`, `gameStarted`, `authoritativeFrame`, `correction`, `gameFinished`, `error`, `pong`
...
The important semantic detail is that `head` and `direction` mean "where the snake moves from for this tile tick", not "the tile the snake should eventually reach".
...
2. Send the input immediately over WebSocket.
...
Remote players continue in their last authoritative direction until a newer authoritative frame says otherwise. Their predicted input timeline contains only "continue direction" entries generated by the client.
